...
class PMTWidgetUI(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pmt_viewbox.scene().sigMouseClicked.connect(self.generate_poly_roi)
        self.pmt_viewbox.scene().sigMouseClicked.connect(self.update_pixel_coords_and_intensity)
        ...
        self.roi = pg.PolyLineROI([[0, 0], [80, 0], [80, 80], [0, 80]], closed=True, pen=self.ROIpen)  # , maxBounds=r1
        self.roi.sigHoverEvent.connect(lambda: self.show_handle_num(self.roi))  # update handle numbers
        ...
        self.go_to_first_handle_button = StylishQT.GeneralFancyButton(label = "Go to 1st point")
        self.go_to_first_handle_button.setFixedHeight(32)
        self.go_to_first_handle_button.clicked.connect(self.go_to_first_point)
        self.go_to_first_handle_button.setToolTip("Set galvo initial positions in advance")
        ...
        self.pmtContourLayout.addWidget(self.regenerate_roi_handle_button, 2, 4)
        ...
        self.add_roi_to_stack_button.setToolTip("Add current ROI to the stack at the contour index")
        ...
        self.del_roi_in_stack_button = StylishQT.stop_deleteButton("Delete ROI")
        self.del_roi_in_stack_button.setToolTip("Delete current ROI from the stack at the contour index")
        ...
        self.reset_roi_stack_button.setToolTip("Clear all the ROIs in the stack")
        ...
        self.generate_contour_scan = StylishQT.generateButton("Generate contour")
        self.pmtContourLayout.addWidget(self.generate_contour_scan, 2, 0)
        self.generate_contour_scan.clicked.connect(lambda: self.generate_final_contour_signals())
        ...
        self.do_contour_scan.clicked.connect(lambda: self.buttonenabled("contourscan", "start"))
        self.do_contour_scan.clicked.connect(lambda: self.measure_pmt_contourscan())
        ...
        depth_label.setToolTip("In case of not changing z-position, set here to 0.")
        ...
        self.move_galvo_button.clicked.connect(lambda: self.move_galvos_to_specified_point(self.desired_galvo_x_spinbox.value(), self.desired_galvo_y_spinbox.value()))
        ...
        pmt_image_inspection_widget = StylishQT.roundQGroupBox(title="PMT-image inspection")
        ...
        pmt_image_inspection_layout.addWidget(self.pmt_image_x_coordinate_label, 0, 0)
        pmt_image_inspection_layout.addWidget(self.pmt_image_y_coordinate_label, 0, 1)
        pmt_image_inspection_layout.addWidget(self.pmt_image_intensity_label, 0, 2)
        ...
        pmt_image_inspection_layout.addWidget(self.tiff_directory_textbox, 1, 1)
        ...
        scanning_and_inspection_container.setLayout(scanning_and_inspection_layout)
    ...
    def generate_poly_roi(self, event):
        ...
        if len(self.clicked_points_list) == 1:
            ...
            self.click_poly_roi = pg.PolyLineROI(positions=[self.starting_point, point])
            self.click_poly_roi.sigHoverEvent.connect(lambda: self.show_handle_num(self.click_poly_roi))  # update handle numbers
    ...
    def measure_pmt_contourscan(self):
        ...
        print("Contour frequency:  %.4f Hz" % contour_freq)
    ...
    def show_handle_num(self, roi_item):
        ...
        self.pmt_handlenum_Label.setText("Handle number: %.d" % self.number_of_ROI_handles)

    def regenerate_roi_handles(self):
        """Regenerate the handles from desired roi in sequence."""

        current_roi_handles_list = self.contour_ROI_handles_dict["handles_{}".format(self.roi_index_spinbox.value())]

        self.pmt_viewbox.removeItem(self.click_poly_roi)

        self.click_poly_roi = pg.PolyLineROI(current_roi_handles_list, pen=self.ROIpen, closed = True)
        self.click_poly_roi.sigHoverEvent.connect(lambda: self.show_handle_num(self.click_poly_roi))
    ...
    def add_coordinates_to_list(self):
        ...
        has_camera_generated_contour_voltages = hasattr(self, 'camera_generated_contour_voltages') and self.camera_generated_contour_voltages is not None
        has_click_poly_roi = hasattr(self, 'click_poly_roi') and self.click_poly_roi is not None

        if has_camera_generated_contour_voltages and has_click_poly_roi == False:
            print("Adding camera generated coordinates to the list.\n")
            self.current_stacked_voltage_signals = self.camera_generated_contour_voltages

        elif has_click_poly_roi and has_camera_generated_contour_voltages == False:
            print("Adding UI-PMT coordinates to the list.\n")
            ...
            self.contour_ROI_handles_dict[ "handles_{}".format(self.roi_index_spinbox.value()) ] = roi_handles_scene_list

        elif has_camera_generated_contour_voltages and has_click_poly_roi:
            print("Trying to add a camera generated contour as well as a UI-PMT contour at the same time. This is not allowed.\n")

        else:
            print("Missing contour coordinates. Please generate or create a contour.\n")

        # Place the signals to the corresponding dictionary position
        self.contour_ROI_signals_dict[ "roi_{}".format(self.roi_index_spinbox.value()) ] = self.current_stacked_voltage_signals


    def del_coordinates_from_list(self):
        """Remove the last mask from the list."""
        del self.contour_ROI_signals_dict[ "roi_{}".format(self.roi_index_spinbox.value()) ]
        del self.contour_ROI_handles_dict[ "handles_{}".format(self.roi_index_spinbox.value()) ]
    ...
    def generate_final_contour_signals(self):
        ...
        if len(self.contour_ROI_signals_dict) == 0:
            QMessageBox.warning(self, "generate_final_contour_signals","No contour coordinates found. Please add some coordinates.\n")
            return
    ...
    def move_galvos_to_specified_point(self, x, y):
        ...
        print("Moving the galvos to: {}, {}".format(x, y))

        self.Value_xPixels = int(self.Scanning_pixel_num_combobox.value())
        self.Value_voltXMax = self.continuous_scanning_Vrange_spinbox.value()

        x_voltage_galvo, y_voltage_galvo = self.convert_coordinates_to_voltage(self.Value_xPixels, self.Value_voltXMax, 1, np.array([[x, y]]))

        print("Corresponding voltages: {}, {}".format(x_voltage_galvo, y_voltage_galvo))
    ...
    def reset_coordinates_dict(self):
        ...
        if hasattr(self, 'camera_generated_contour_voltages') and self.camera_generated_contour_voltages is not None:
            self.camera_generated_contour_voltages = None
    ...
    def generate_contour_coordinates(self, roi_item):
        ...
        self.viewbox_handle_positions = [handle.pos() for handle in self.ROI_handles]
        self.viewbox_handle_coords = np.vstack([ [pos.x(), pos.y()] for pos in self.viewbox_handle_positions])
        ...
        if self.contour_strategy.currentText() == "Evenly between":
            if contour_length % self.number_of_ROI_handles == 0:
                self.points_per_handle = contour_length // self.number_of_ROI_handles
            else:
                QMessageBox.warning(self, "generate_contour_coordinates", "Please input a multiple of the handle number for the contour points.")
                return

            evenly_expanded_viewbox_coords = self.interpolate_handles_evenly(self.viewbox_handle_coords, self.points_per_handle)
        ...
        if self.contour_strategy.currentText() == "Uniform":
            uniformly_expanded_viewbox_coords = self.interpolate_uniformly(self.viewbox_handle_coords, contour_length)
        ...
        self.expanded_viewbox_x_coords, self.expanded_viewbox_y_coords = self.convert_coordinates_to_voltage(
            self.Value_xPixels,
            self.Value_voltXMax,
            contour_length,
        ...
        if self.is_galvo_speed_and_acceleration_within_limits(self.Daq_sample_rate_pmt, self.expanded_viewbox_x_coords, self.expanded_viewbox_y_coords):

            # The final stacked voltage signals, ready to be sent to the DAQ
            current_stacked_voltage_signals = np.vstack((self.expanded_viewbox_x_coords, self.expanded_viewbox_y_coords))
    ...
    def interpolate_handles_evenly(self, handle_positions, points_per_handle):
        ...
        def interpolate_between_two_points(p1, p2, num_points):
            ...
            return np.array([[p1[0] + i * x_step, p1[1] + i * y_step] for i in range(1, num_points)])
        ...
        for i in range(handle_positions.shape[0] - 1):
            ...
            interpolated_points.extend(interpolate_between_two_points(p1, p2, points_per_handle))
    ...
    def interpolate_uniformly(self, handle_positions, contour_length):
        ...
        tck, _ = splprep([handle_positions[:, 0], handle_positions[:, 1]], s=0, per=True)
    ...
    def convert_coordinates_to_voltage(self, Value_xPixels, Value_voltXMax, contour_length, viewbox_coords):
        """Convert coordinates to voltages."""

        if Value_xPixels == 500:
            print(f"Converting coordinates within range [-{Value_voltXMax}, {Value_voltXMax}] V. ")

            # Scale coordinates from pixels to voltages
            viewbox_coords[:, 0] = ((viewbox_coords[:, 0] / Value_xPixels) * 2 - 1) * Value_voltXMax
            viewbox_coords[:, 1] = ((viewbox_coords[:, 1] / Value_xPixels) * 2 - 1) * Value_voltXMax
            ...
            QMessageBox.warning(self, "convert_coordinates_to_voltage", "Please input the correct pixel number for the conversion.")
            return

        return transformed_x, transformed_y

    def is_galvo_speed_and_acceleration_within_limits(self, sampling_rate, trace_x, trace_y):
        ...
        def log_and_display(value, label, unit, scientific_notation="0f"):
            ...
            print(f"Mean {label}: {mean_value:.{scientific_notation}} [{unit}]")
            return mean_value
        ...
        max_acceleration = constants.maxGalvoAccel  # Maximum galvo acceleration (Volt/s^2)

        # Compute speed and acceleration
        contour_x_speed, contour_x_acceleration = calculate_speed_acceleration(trace_x, time_gap)
        contour_y_speed, contour_y_acceleration = calculate_speed_acceleration(trace_y, time_gap)
        ...
        log_and_display(contour_x_acceleration, "x acceleration", "volt/s^2", "2e")
        log_and_display(contour_y_acceleration, "y acceleration", "volt/s^2", "2e")
        ...
        exceeded_accelerations = {
            "X": np.where(np.abs(contour_x_acceleration) > max_acceleration)[0] + 1,
            "Y": np.where(np.abs(contour_y_acceleration) > max_acceleration)[0] + 1
        }

        if not exceeded_accelerations["X"].size and not exceeded_accelerations["Y"].size:
        ...
        return not (exceeded_speeds["X"].size or exceeded_speeds["Y"].size or exceeded_accelerations["X"].size or exceeded_accelerations["Y"].size)
    ...
    def displayTiffImage(self, file_path):
        try:
            ...
            self.pmt_viewbox.setLimits(xMin=0, xMax=tiff_image.shape[1], yMin=0, yMax=tiff_image.shape[0])
    ...
    def update_pixel_coords_and_intensity(self, event):
        ...
        def show_pixel_coords():
            x, y = int(event.pos().x()), int(event.pos().y())
            qpoint_viewbox = self.pmt_viewbox.mapSceneToView(QPoint(x, y))
            self.pmt_image_x_coordinate_label.setText("X-coordinate: %.0f" % qpoint_viewbox.x())
            self.pmt_image_y_coordinate_label.setText("Y-coordinate: %.0f" % qpoint_viewbox.y())
            return int(qpoint_viewbox.x()), int(qpoint_viewbox.y())

        if hasattr(self, 'data_pmtcontineous'):
            x, y = show_pixel_coords()
            self.pmt_image_intensity_label.setText("Intensity: %.4f" % self.data_pmtcontineous[y, x])
        elif hasattr(self, 'pmtimgroi') and self.pmtimgroi.image is not None:
            x, y = show_pixel_coords()
            self.pmt_image_intensity_label.setText("Intensity: %.4f" % self.pmtimgroi.image[y, x])
    ...
    def handle_received_camera_contour(self, contour):
        ...
        if hasattr(self, 'scatter_plot') and self.scatter_plot is not None:
            self.pmt_viewbox.removeItem(self.scatter_plot)
            print("Removed previous contour plot")

        # Scatter plot the points
        scatter_points = [{'pos': (x, y), 'size': 2, 'pen': {'color': 'r', 'width': 2}, 'brush': 'r'} for x, y in contour]
        ...
        converted_x_coords, converted_y_coords = self.convert_coordinates_to_voltage(self.Value_xPixels, self.Value_voltXMax, len(contour), contour)

        self.Daq_sample_rate_pmt = int(self.contour_samplerate.value())

        if self.is_galvo_speed_and_acceleration_within_limits(self.Daq_sample_rate_pmt, converted_x_coords, converted_y_coords):
            self.camera_generated_contour_voltages = np.vstack((converted_x_coords, converted_y_coords))
    ...
    def emit_contour_signal(self):
        ...
        self.SignalForContourScanning.emit(
            ...
            self.Daq_sample_rate_pmt,
            (1 / int(self.contour_samplerate.value()) * 1000) * self.points_per_round, # time per contour scan
