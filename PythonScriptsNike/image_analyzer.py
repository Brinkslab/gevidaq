...
class ImageAnalyzer:
    ...
    def update_window_size(self, val):
            contour = self.ui_contour if hasattr(self, 'ui_contour') else self.base_contour
    ...
    def update_plots(self, contour, window_size, scaling):
            ...
            self.smoothened_curve_plot.set_data(scaled_contour[:, 1], scaled_contour[:, 0])
            plt.draw()
    ...
    def resize_contour(self, contour, scaling_factor):
        ...
        # Scale the polygon
        scaled_polygon = scale(polygon, xfact=scaling_factor, yfact=scaling_factor, origin=centroid)
    ...
    def display_multiple_contours(self, ax, image, minimum_intensity, maximum_intensity, number_of_contours):
        """Display multiple contours in the image based on the given intensity thresholds."""
        colors = plt.colormaps.get_cmap("Blues")(np.linspace(0.3, 1, number_of_contours+1))

        for i in range(number_of_contours+1):
            threshold = int(minimum_intensity + i * (maximum_intensity - minimum_intensity) / number_of_contours)
            largest_contour = self.find_contour(image, threshold)
            ax.plot(largest_contour[:, 1], largest_contour[:, 0], color=colors[i], linewidth=1.5, label='Intensity = ' + str(threshold))
    ...
    def zoom_in_on_roi(self, ax, contour, factor = 1.25):
        ...
        x_left, x_right = int(np.min(contour[:, 1])), int(np.max(contour[:, 1]))
        x_diff = x_right - x_left
        y_top, y_bottom = int(np.min(contour[:, 0])), int(np.max(contour[:, 0]))
        y_diff = y_bottom - y_top
        x_centroid, y_centroid = (x_left + x_right) // 2, (y_top + y_bottom) // 2
    ...
    def auto_level_image(self, image, lower_bound, upper_bound):
        ...
        return (image - lower_bound) / (upper_bound - lower_bound) * upper_bound

    def display_tiff_image(self, ax, lower_bound, upper_bound, alpha=1.0):
        """Display the TIFF image with auto-leveling."""
        leveled_image = self.auto_level_image(self.tiff_image, lower_bound, upper_bound)
        ax.imshow(leveled_image, cmap="gray", aspect="equal", alpha=alpha)


if __name__ == "__main__":
    image_analyzer = ImageAnalyzer(r"M:/tnw/ist/do/projects/Neurophotonics/Brinkslab/People/Xin Meng/Code/Python_test_TF2/cell0000.TIF")
    ...
    lower_bound, upper_bound = image_analyzer.determine_thresholds(image_analyzer.tiff_image)
    image_analyzer.base_contour = image_analyzer.find_contour(image_analyzer.tiff_image, image_analyzer.intensity_threshold)
    image_analyzer.base_smoothened_curve = image_analyzer.smoothen_contour(image_analyzer.base_contour, 10)
    ...
    image_analyzer.display_multiple_contours(axes[0, 1], image_analyzer.tiff_image, 2000, 6000, 4)
    ...
    image_analyzer.varying_intensity_plot, = axes[1, 0].plot(image_analyzer.base_contour[:, 1], image_analyzer.base_contour[:, 0], color='#AFEEEE', linewidth=1)
    image_analyzer.zoom_in_on_roi(axes[1, 0], image_analyzer.base_contour)

    # Create a horizontal slider for threshold adjustment
    ax_threshold = plt.axes([0.12, 0.02, 0.2, 0.03], facecolor='lightgoldenrodyellow')
    image_analyzer.slider_threshold = Slider(ax_threshold, 'Intensity', lower_bound, upper_bound//2, valinit=image_analyzer.intensity_threshold, valstep=50)
    ...
    image_analyzer.smoothened_curve_plot, = axes[1, 1].plot( image_analyzer.base_smoothened_curve[:, 1], image_analyzer.base_smoothened_curve[:, 0], color='#AFEEEE', linewidth=1)
    ...
    ax_window_size = plt.axes([0.48, 0.02, 0.2, 0.03], facecolor='lightgoldenrodyellow')
    image_analyzer.slider_window_size = Slider(ax_window_size, 'Smoothness', 1, 20, valinit=10, valstep=1)

    #  Create a vertical slider for scaling the contour
    ax_scaling = plt.axes([0.70, 0.1, 0.02, 0.3], facecolor='lightgoldenrodyellow')
    image_analyzer.slider_scaling = Slider(ax_scaling, 'Scaling', 0.5, 1.5, valinit=1, valstep=0.05, orientation='vertical')

    # Update function for the sliders
    image_analyzer.slider_threshold.on_changed(image_analyzer.update_threshold)
    image_analyzer.slider_window_size.on_changed(image_analyzer.update_window_size)
